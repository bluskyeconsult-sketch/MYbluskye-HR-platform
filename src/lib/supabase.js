...
// Export initialized client (async for safety)
...
/**
 * Check if session is valid (simplified)
...
/**
 * Get current session (alias for recoverSession)
...
export function initAuthListener() {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
        ...
        authEventListeners.forEach((callback, id) => {
            try {
                callback(event, session);
            } catch (err) {
                console.error(`Auth listener error (${id}):`, err);
            }
        });
    ...
    try {
        const response = await fetch(`/api/index?action=${action}`, {
            ...
            headers: {
                ...
                'Authorization': `Bearer ${(await getCurrentUser())?.id || ''}`
            },
