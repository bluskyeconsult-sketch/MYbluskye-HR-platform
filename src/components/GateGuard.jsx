...
export function GateGuard({ 
    ...
    useEffect(() => {
        ...
        if (useAsync && action) {
            setIsChecking(true);
            // FIXED: the async check has been observed hanging
            // indefinitely (isChecking staying true forever, confirmed via
            // live diagnostic). A 5-second timeout guarantees isChecking
            // and asyncAllowed always settle, falling back to the
            // already-reliable syncAllowed rather than blocking the UI
            // permanently on a promise that may never resolve or reject.
    ...
    // FIXED: the async check has now proven unreliable twice in production
    // (first a stale-overwrite race, now an apparently hanging promise that
    // never resolves or rejects, confirmed via isChecking staying true
    // indefinitely). Rather than trust an async result that can get stuck,
    // access is granted if EITHER check allows it - the sync check is
    // never overridden by an async result that failed to settle or is
    // still pending. The async check can still explicitly grant access in
    // cases the sync map doesn't cover, but it can no longer block access
    // the sync check already, correctly confirmed.
    const hasAccess = syncAllowed || asyncAllowed === true;
