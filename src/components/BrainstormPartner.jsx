...
import { useState, useEffect, useRef } from 'react';// src/components/BrainstormPartner.jsx
// ODUSBABA BRAINSTORM PARTNER v3.1 - PRODUCTION READY
// ✅ AI-powered product development assistant via Unified API
// ✅ Idea generation, deep dive analysis, follow-up questions
// ✅ Save ideas, copy to clipboard, conversation history
// ✅ Fallback ideas for offline scenarios
//
// FIXED (2026-08-16): all three call sites (generateIdeas, handleDeepDive,
// handleFollowUp) sent {messages: [...], temperature, maxTokens} to the
// real chat handler — which requires a single `message` string field and
// returns 400 without it. This is the exact same bug found and fixed in
// ODUSBABAChat.jsx earlier this session (the biggest fix of the whole
// project) — this component has very likely never returned a real AI
// response either, always silently falling to the local fallback ideas
// instead. Fixed to send {message, systemPrompt, temperature, maxTokens},
// matching the real handler's actual shape.
//
// Also fixed a prompt/parsing mismatch: the AI was asked for "bullet
// points" but the response parser expects a numbered list
// (split(/\d+\.\s+/)) — now the prompts explicitly ask for a numbered
// list, matching what's actually being parsed.
//
// This component was also never mounted anywhere in App.jsx (like several
// others found this session) and has no internal admin-only check despite
// being an internal team tool — gated to admin/super_admin at the mount
// point in App.jsx rather than inside this file, matching how other
