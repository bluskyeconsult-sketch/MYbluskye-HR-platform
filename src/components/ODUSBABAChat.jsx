...
export default function ODUSBABAChat() {
    ...
    // FIXED (2026-09-13): confirmed via prior session history that this
    // was honestly flagged on 2026-08-07 as unbuilt - despite its name,
    // this never actually fetched or referenced any real content, just
    // returned a hardcoded template with static links. Now calls the
    // real backend action, which grounds the answer in actual cached
    // content from approved law/immigration sources.
    async function fetchLegalInfo(countryCode, topic) {
        try {
            const response = await fetch('/api/index?action=odusbaba-legal-query', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ question: topic, countryCode })
            });
            const data = await response.json();
            if (data.success) {
                return data.answer;
            }
            throw new Error(data.error || 'Legal query failed');
        } catch (error) {
            console.warn('Legal query failed, falling back to source directory:', error);
