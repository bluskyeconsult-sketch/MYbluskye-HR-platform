...
// COMPLETE VIRTUAL ASSISTANT PAGE - Full AI integration, credit tracking, task history, 24 assistants
...
export default function HireVirtualAssistant() {
    ...
    const categories = [
        { id: 'all', name: 'All Assistants', icon: Bot, color: 'primary' },
        ...
        <div className="min-h-screen bg-slate-950 py-12">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex border-b border-slate-800 mb-6">
                    <button onClick={() => setActiveTab('assistants')} className={`px-4 py-2 text-sm font-medium transition ${activeTab === 'assistants' ? 'text-primary-400 border-b-2 border-primary-400' : 'text-slate-400 hover:text-white'}`}>
                        <Bot className="w-4 h-4 inline mr-1" /> Virtual Assistants ({VIRTUAL_ASSISTANTS.length})
                    </button>
                    <button onClick={() => setActiveTab('history')} className={`px-4 py-2 text-sm font-medium transition ${activeTab === 'history' ? 'text-primary-400 border-b-2 border-primary-400' : 'text-slate-400 hover:text-white'}`}>
                    ...
                    <>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 mb-6">
                            <div className="flex-1 relative">
                                ...
                                <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search assistants by name or description..." className="w-full pl-10 pr-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500" />
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {categories.map(cat => {
                                    const Icon = cat.icon;
                                    return (
                                        <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`px-3 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1 ${selectedCategory === cat.id ? 'bg-primary-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                            {filteredVAs.map(va => (
                                <div key={va.id} className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 hover:-translate-y-1 transition-all cursor-pointer hover:border-primary-500/50 group" onClick={() => { setSelectedVA(va); setShowModal(true); setOutput(null); setInput(''); }}>
                            ...
                            taskHistory.map(task => {
                                ...
                                return (
                                    <div key={task.id} className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 hover:border-slate-700 transition">
                                        ...
                                        {isExpanded && task.output && <div className="mt-3 p-3 bg-slate-800/50 rounded-lg"><p className="text-slate-300 text-sm whitespace-pre-wrap">{task.output}</p></div>}
                                        <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-800">
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => submitFeedback(task.id, 'positive')} className={`text-xs flex items-center gap-1 px-2 py-1 rounded transition ${feedback[task.id] === 'positive' ? 'bg-emerald-500/20 text-emerald-400' : 'text-slate-500 hover:text-emerald-400'}`}><ThumbsUp className="w-3 h-3" /> Helpful</button>
                                                <button onClick={() => submitFeedback(task.id, 'negative')} className={`text-xs flex items-center gap-1 px-2 py-1 rounded transition ${feedback[task.id] === 'negative' ? 'bg-red-500/20 text-red-400' : 'text-slate-500 hover:text-red-400'}`}><ThumbsDown className="w-3 h-3" /> Not Helpful</button>
                                            </div>
                                            {task.output && !isExpanded && <button onClick={() => setExpandedTask(task.id)} className="text-xs text-primary-400 hover:text-primary-300 flex items-center gap-1">View result <ChevronRight className="w-3 h-3" /></button>}
                                        </div>
                                    </div>
                                );
                            })
                    ...
                    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                        <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-lg w-full p-6">
                            ...
                            <form onSubmit={handleExecute} className="space-y-4">
                                <textarea value={input} onChange={(e) => setInput(e.target.value)} rows={5} placeholder={`Describe what you need help with...\n\nExample: "I'm applying for a Senior Software Engineer role at Google. Please help optimize my CV."`} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500" required />
                                <button type="submit" disabled={loading || (eligibility && !eligibility.isUnlimited && eligibility.remaining < selectedVA.price)} className="w-full py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition disabled:opacity-50 flex items-center justify-center gap-2">
                            ...
                            {output && (
                                <div className="mt-4 p-3 bg-slate-800/50 rounded-lg">
                                    ...
                                    <button onClick={() => navigator.clipboard.writeText(output)} className="mt-2 text-xs text-primary-400 hover:text-primary-300 flex items-center gap-1"><Copy className="w-3 h-3" /> Copy</button>
                                </div>
                            )}
                        </div>
                    </div>
