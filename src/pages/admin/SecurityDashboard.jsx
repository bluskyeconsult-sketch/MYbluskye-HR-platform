...
// Security Dashboard - Threat monitoring and IP management

import { useState, useEffect } from 'react';
import { supabase } from '../../lib/supabase';
import { Shield, AlertTriangle, Ban, Eye, Loader2, CheckCircle, XCircle, RefreshCw, Plus, Trash2, Clock, Activity } from 'lucide-react';
...
export default function SecurityDashboard() {
    ...
    async function loadData() {
        ...
        try {
            ...
            setStats({ total: eventsData?.length || 0, critical });
        } catch (error) {
        ...
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-400">Monitor security events and manage blocked IPs</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                    <div className="flex items-center justify-between">
                        <div><p className="text-slate-400 text-sm">Total Events</p><p className="text-2xl font-bold text-white">{stats.total}</p></div>
                        <Shield className="w-8 h-8 text-primary-400 opacity-50" />
                    </div>
                </div>
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                    <div className="flex items-center justify-between">
                        <div><p className="text-slate-400 text-sm">Critical Events</p><p className="text-2xl font-bold text-red-400">{stats.critical}</p></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5">
                    ...
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                        ...
                        {events.length === 0 && <p className="text-slate-400 text-center py-4">No security events recorded</p>}
                    </div>
                </div>
