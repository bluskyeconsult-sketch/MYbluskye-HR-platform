...
import { Briefcase, Search, Loader2, CheckCircle, XCircle, Eye, Trash2, Building2, MapPin, DollarSign } from 'lucide-react';
        ...
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-slate-400">View and moderate all job listings</p>
                </div>
                    ...
                    <table className="w-full">
                        <thead className="bg-slate-800/50 border-b border-slate-800">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-white text-sm">Company</th>
                                <th className="px-4 py-3 text-left text-white text-sm">Location</th>
                                <th className="px-4 py-3 text-left text-white text-sm">Posted</th>
                        ...
                        <tbody>
                            {filteredJobs.map(job => (
                                <tr key={job.id} className="border-b border-slate-800 hover:bg-slate-800/30">
                                    <td className="px-4 py-3 text-white text-sm">{job.title}</td>
                                    <td className="px-4 py-3 text-slate-300 text-sm">{job.company}</td>
                                    <td className="px-4 py-3 text-slate-300 text-sm">{job.location || 'Remote'}</td>
                                    <td className="px-4 py-3 text-slate-400 text-sm">{new Date(job.created_at).toLocaleDateString()}</td>
                                    <td className="px-4 py-3">
                                        {job.is_active ? (
                                            <span className="text-emerald-400 text-sm flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Active</span>
                                        ) : (
                                            <span className="text-red-400 text-sm flex items-center gap-1"><XCircle className="w-3 h-3" /> Inactive</span>
                                        )}
                                    </td>
                                    <td className="px-4 py-3">
