...
import { Play, CheckCircle, Clock, BookOpen, Volume2, User, Star, Loader2 } from 'lucide-react';
import { getCourse, enrollInCourse, getUserEnrollment, updateModuleProgress, addCourseReview } from '../services/courseService';
                ...
                <div className="max-w-7xl mx-auto px-4 py-12">
                    <div className="flex flex-col lg:flex-row gap-8">
                        <div className="flex-1">
                            <div className="flex items-center gap-2 text-sm text-slate-400 mb-2"><span>{course.category || 'General'}</span><span>•</span><span className="capitalize">{course.level}</span></div>
                            ...
                            <div className="flex flex-wrap gap-4 text-sm">
                                ...
                                <span className="flex items-center gap-1 text-yellow-400"><Star className="w-4 h-4 fill-yellow-400" /> {course.rating || 0}</span>
                        ...
                        <div className="w-full lg:w-80">
                            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-center sticky top-24">
                                ...
                                {!isEnrolled ? (<button onClick={handleEnroll} className="mt-4 w-full py-3 bg-primary-500 text-white rounded-lg font-semibold hover:bg-primary-600 transition-colors">Enroll Now</button>) : isCompleted ? (<div className="mt-4 p-3 bg-success/20 text-success rounded-lg"><CheckCircle className="w-5 h-5 inline mr-2" /> Course Completed!</div>) : (<div className="mt-4"><div className="w-full bg-slate-700 rounded-full h-2 mb-2"><div className="bg-primary-500 h-2 rounded-full transition-all" style={{ width: `${progress}%` }}></div></div><p className="text-slate-400 text-sm">{progress}% Complete</p></div>)}
                            </div>
                        </div>
                    </div>
                </div>
