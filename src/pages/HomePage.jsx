...
// Features: Real-time stats, trust badges, country job counts, latest articles, animated sections
...
export default function HomePage() {
  ...
  async function fetchStats() {
    try {
      // Get active users (total profiles)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-950 overflow-x-hidden">
      ...
      {/* Cinematic Text Advert After HeroSection */}
      <CinematicTextAdvert />
      ...
      <div ref={statsRef} id="stats-section" className="max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-8">
        ...
        >
          {statsCards.map((stat, idx) => (
            <motion.div key={idx} variants={itemVariants} className={`${stat.bgColor} border border-slate-700 rounded-xl p-6 text-center hover:border-${stat.color}-500/30 transition-all hover:-translate-y-1`}>
